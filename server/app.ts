...
import { eventEmitter, getRecentEvents } from './lib/reactive-stream'
...
app.use('/api/*', cors({
  origin: (origin) => {
    ...
    // Allow Vercel preview/production deployments (same-origin, CORS not needed but be safe)
    if (origin?.endsWith('.vercel.app')) return origin
    return null
  },
}))

// ── Nonce store for session activation replay prevention ──
const usedChallenges = new Map<string, number>()
setInterval(() => {
  const now = Date.now()
  usedChallenges.forEach((ts, key) => {
    if (now - ts > 120_000) usedChallenges.delete(key)
  })
}, 60_000).unref?.()
...
app.post('/api/chat', async (c) => {
  ...
  const trimmedMessages = messages
    .slice(-MAX_MESSAGES)
    // Strip any injected system-role messages — only allow user and assistant
    ...
    .map((m: UIMessage) => ({
      ...
      content: typeof m.content === 'string'
        ...
        : Array.isArray(m.content)
          ? (m.content as Array<{ type: string; text?: string; [k: string]: unknown }>).map(part =>
              part.type === 'text' && typeof part.text === 'string'
                ? { ...part, text: part.text.slice(0, MAX_MSG_CHARS) }
                : part
            )
          : m.content,
    }))
...
app.post('/api/session', async (c) => {
  ...
  const challengeKey = `${ownerAddress.toLowerCase()}:${timestamp}`
  if (usedChallenges.has(challengeKey)) return c.json({ error: 'Challenge already used. Try again.' }, 400)
  ...
  usedChallenges.set(challengeKey, Date.now())
...
app.post('/api/session/revoke', async (c) => {
  ...
  const { success: addrOk } = rateLimit(`session-revoke-addr:${body.ownerAddress.toLowerCase()}`, 3, 60_000)
  if (!addrOk) return c.json({ error: 'Rate limit reached.' }, 429)
...
app.post('/api/session/status', async (c) => {
  ...
  try { body = await c.req.json() } catch { return c.json({ active: false }) }
  if (!body.address || !isAddress(body.address)) return c.json({ active: false })
  return c.json(getSessionStatus(body.address as `0x${string}`))
})
