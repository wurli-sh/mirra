...
export function LeaderModal({ mode, open, onClose, onSuccess }: Props) {
  ...
  const {
    ...
    isSuccess: deregSuccess,
    error: deregError,
  } = useDeregisterLeader();

  const isRegister = mode === "register";
  const loading = isRegister
    ? regPending || regConfirming
    : deregPending || deregConfirming;
          ...
          >
            ...
            <div className="px-6 pt-6 pb-4 flex items-start justify-between">
              <div className="flex items-center gap-3">
                <div
                  className={`size-10 rounded-lg flex items-center justify-center ${isRegister ? "bg-primary/20" : "bg-danger/10"}`}
                >
                  {isRegister ? (
                ...
                <div>
                  <h2 className="font-bold text-lg">
                    {isRegister ? "Become a Leader" : "Deregister Leader"}
                  </h2>
                  <p className="text-xs text-text-muted">
                    {isRegister
                      ? "Stake STT to let others mirror you"
                      : "Unstake your STT and stop leading"}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="px-6 pb-6">
                ...
                <>
                  ...
                  <div className="flex items-center gap-2 bg-warning/10 text-warning rounded-lg px-3.5 py-2.5 mb-5">
                    ...
                    <span className="text-xs font-medium">
                      This will stop all mirroring for your followers.
                    </span>
                  </div>
                </>
              )}
              ...
              <motion.button
                ...
                disabled={
                  loading ||
                  success ||
                  (isRegister && (!stakeAmount || Number(stakeAmount) < 10))
                }
                whileTap={{ scale: 0.97 }}
              >
                ...
                {isRegister
                  ? loading
                    ...
                    : success
                      ? "Registered!"
                      : `Stake ${stakeAmount || "10"} STT & Register`
                  : loading
                    ? "Deregistering..."
                    : success
                      ? "Done!"
