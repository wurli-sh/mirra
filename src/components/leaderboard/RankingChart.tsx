...
function positionsToPath(positions: number[], width: number, height: number, padding: number): string {
    ...
    <div className="border border-border rounded-xl overflow-hidden">
      ...
      <div className="px-6 pt-4 pb-2">
        ...
        >
          ...
          {lines.map((line) => {
            const d = positionsToPath(line.positions, svgWidth, svgHeight, padding)
            ...
            return (
              <g key={line.leader}>
                ...
                <circle cx={lastX} cy={lastY} r={7} fill={line.color} fillOpacity={0.15} />
              </g>
            )
          })}
        </svg>
      ...
      <div className="flex items-center gap-5 px-6 py-4 border-t border-border/40">
        {lines.map((line) => (
          <div key={line.leader} className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-text-faint">#{line.rank}</span>
          </div>
        ))}
      </div>
    </div>
