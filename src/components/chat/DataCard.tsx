...
import {
  useReactiveEvents,
  type ReactiveEvent as SSEEvent,
} from "@/hooks/useReactiveEvents";
...
function LeaderboardCard({
  ...
  return (
    <Card
      icon={Trophy}
      title="Leaderboard"
      subtitle={`${count} leader${count !== 1 ? "s" : ""}`}
    >
      {leaders.length === 0 ? (
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="text-text-muted border-b border-primary/15">
                ...
                <th className="text-right font-medium py-1.5 pr-4 pl-2">
                  Followers
                </th>
              </tr>
            </thead>
            <tbody>
              {leaders.map((l, i) => (
                ...
                >
                  ...
                  <td className="py-2 px-2 font-mono text-text">
                    {l.shortAddress ?? truncateAddr(l.address)}
                  </td>
                  <td className="py-2 px-2 text-right font-semibold text-text">
                    {l.score}
                  </td>
                  <td
                    className={`py-2 px-2 text-right font-semibold ${l.pnl >= 0 ? "text-success" : "text-danger"}`}
                  >
                    {formatPnl(l.pnl)}
                  </td>
                  ...
                  <td className="py-2 pr-4 pl-2 text-right text-text-muted">
                    {l.followers}
                  </td>
                </tr>
              ))}
...
function LeaderStatsCard({ data }: { data: Record<string, unknown> }) {
  return (
    <Card
      icon={TrendingUp}
      title="Leader Stats"
      subtitle={truncateAddr(data.address as string)}
    >
      <div className="grid grid-cols-2 gap-x-4 gap-y-2">
        ...
        <Stat
          label="PnL"
          value={`${formatPnl(data.pnl as number)} STT`}
          positive={(data.pnl as number) >= 0}
        />
...
function PositionsCard({ positions }: { positions: Position[] }) {
  return (
    ...
    >
      {positions.length === 0 ? (
        <p className="text-xs text-text-muted py-2">
          No active positions. Follow a leader to get started!
        </p>
      ) : (
        <div className="space-y-2.5">
          {positions.map((p) => (
            <div
              key={p.leader}
              className="flex items-center justify-between gap-3 py-1.5 border-b border-primary/10 last:border-0"
            >
              <div className="min-w-0">
                <span className="text-xs font-mono text-text block">
                  {truncateAddr(p.leader)}
                </span>
                <span className="text-xs text-text-muted">
                  Score {p.score} · SL {p.stopLoss} STT
                </span>
              </div>
              <div className="text-right shrink-0">
                ...
                <span
                  className={`text-xs font-semibold ${p.pnl >= 0 ? "text-success" : "text-danger"}`}
                >
                  {formatPnl(p.pnl)} STT ({p.pnlPercent > 0 ? "+" : ""}
                  {p.pnlPercent}%)
                </span>
              </div>
            </div>
  ...
  return (
    <Card
      ...
      subtitle={
        isConnected
          ? "Live"
          : lastTradeSecondsAgo != null
            ? `${lastTradeSecondsAgo}s ago`
            : undefined
      }
    >
      {isConnected && (
      ...
      {displayEvents.length === 0 ? (
        <p className="text-xs text-text-muted py-2">
          No recent trades yet. Swaps will appear here in real time.
        </p>
      ) : (
        <div className="space-y-1.5">
          {displayEvents.slice(0, 5).map((e, i) => (
            <div
              key={`${e.timestamp}-${i}`}
              className="flex items-center gap-2 text-xs py-1 border-b border-primary/10 last:border-0"
            >
              <span className="text-xs font-bold uppercase tracking-wider text-success bg-success/10 px-1.5 py-0.5 rounded shrink-0">
                {e.type === "swap" ? "SWAP" : "MIRROR"}
              </span>
              <span className="text-text-muted truncate">
                {truncateAddr(e.leader)}
              </span>
...
function Card({
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 bg-surface">{children}</div>
    </motion.div>
  );
}
...
function Stat({
  ...
  return (
    <div>
      ...
      <p
        className={`text-sm font-semibold ${positive === true ? "text-success" : positive === false ? "text-danger" : "text-text"}`}
      >
        {value}
      </p>
    </div>
...
export function DataCard({
  ...
  switch (toolName) {
    ...
    case "get_recent_trades":
      return (
        <RecentTradesCard
          events={(data.events as TradeEvent[]) ?? []}
          lastTradeSecondsAgo={data.lastTradeSecondsAgo as number | null}
        />
      );
    default:
      return null;
