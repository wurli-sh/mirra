...
const ACTION_CONFIG: Record<
  ActionType,
  { icon: typeof ArrowRightLeft; label: string; desc: string }
> = {
  swap: {
    icon: ArrowRightLeft,
    label: "Swap",
    desc: "Trade tokens on SimpleDEX",
  },
  follow: {
    icon: UserPlus,
    label: "Follow Leader",
  ...
  unfollow: {
    icon: UserMinus,
    label: "Unfollow",
    desc: "Stop mirroring this leader",
  },
  deposit: {
    icon: ArrowDownToLine,
    label: "Deposit More",
    desc: "Add to your follow position",
  },
  withdraw: {
    icon: ArrowUpFromLine,
    label: "Withdraw",
    desc: "Pull STT from your position",
  },
  register: {
    icon: Shield,
    label: "Register as Leader",
    desc: "Stake STT and let others mirror you",
  },
  deregister: {
    icon: ShieldOff,
    label: "Deregister Leader",
    desc: "Unstake and stop leading",
  },
  claimFees: {
    icon: Coins,
    label: "Claim Fees",
    desc: "Collect your earned leader fees",
  },
  approve: {
    icon: BadgeCheck,
    label: "Approve Token",
    desc: "Allow contract to spend your tokens",
  },
};
...
const FOLLOW_UPS: Record<ActionType, FollowUp[]> = {
  swap: [
    ...
    { icon: Wallet, label: "My balances", text: "Show my token balances" },
    {
      icon: TrendingUp,
      label: "Top leaders",
      text: "Who are the top leaders right now?",
    },
  ],
  follow: [
    {
      icon: TrendingUp,
      label: "My positions",
      text: "Show my follow positions",
    },
    {
      icon: UserPlus,
      label: "Follow another",
      text: "Who else should I follow?",
    },
  ],
  unfollow: [
    {
    ...
    },
    {
      ...
      text: "Who are the top leaders to follow?",
    },
  ],
  deposit: [
    {
  ...
  approve: [
    {
      icon: UserPlus,
      label: "Follow a leader",
      text: "Who are the top leaders to follow?",
    },
    {
      icon: ArrowDownToLine,
      label: "Deposit",
      text: "I want to deposit more to my position",
    },
  ],
};
...
function makeCardKey(data: ActionCardData): string {
  ...
  if (data.type === "swap")
    return `swap-${data.tokenIn}-${data.tokenOut}-${data.amountIn}`;
...
export function ActionCard({ data, onQuickAction }: Props) {
  ...
  useEffect(() => {
    if (isPending) {
      toast.loading(`${label} — waiting for wallet...`, {
        id: `tx-${data.type}`,
      });
      setStatus("confirming");
    } else if (isConfirming) {
      toast.loading(`${label} — confirming on-chain...`, {
        id: `tx-${data.type}`,
      });
      setStatus("confirming");
    }
  ...
  const handleConfirm = useCallback(() => {
    ...
    if (
      ["follow", "unfollow", "deposit", "withdraw"].includes(data.type) &&
      data.leader
    ) {
      ...
      switch (data.type) {
        case "swap": {
          ...
          if (
            ...
            setErrorMsg("Invalid swap amounts.");
            setStatus("error");
            return;
          }
          writeContract({
        ...
        case "follow": {
          ...
          if (
            ...
            setErrorMsg("Invalid follow parameters.");
            setStatus("error");
            return;
          }
          writeContract({
        ...
        case "register": {
          ...
          if (!isFinite(stake) || stake <= 0) {
            setErrorMsg("Invalid stake amount.");
            setStatus("error");
            return;
          }
          writeContract({
        ...
        case "claimFees": {
          const tokenMap: Record<string, Address> = {
            STT: contracts.sttToken,
            USDC: contracts.usdcToken,
          };
          const feeToken =
            tokenMap[(data.token as string) || "STT"] || contracts.sttToken;
          writeContract({
            address: contracts.followerVault,
            abi: FollowerVaultAbi,
  ...
  const config = ACTION_CONFIG[data.type] ?? {
    icon: ArrowRightLeft,
    label: data.type,
  };
  ...
  const rate =
    data.type === "swap" && Number(data.amountIn) > 0
      ...
      <div className="px-4 py-3 bg-surface">
        {data.type === "swap" && (
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <TokenPill
                symbol={data.tokenOut}
                amount={Number(data.estimatedOut).toFixed(4)}
                sublabel="You get"
                highlight
              />
            </div>
            {/* Rate + slippage */}
            <div className="flex items-center justify-between text-xs text-text-muted pt-1 border-t border-primary/10">
              <span>
                Rate: 1 {data.tokenIn} = {rate} {data.tokenOut}
              </span>
        ...
        {data.type === "follow" && (
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <MiniStat
                label="Slippage"
                value={`${(data.slippageBps / 100).toFixed(1)}%`}
              />
        ...
        {(data.type === "deposit" || data.type === "withdraw") && (
          <div className="space-y-2">
            <div className="flex items-center gap-2 bg-primary/10 rounded-md px-3 py-2">
              {data.type === "deposit" ? (
                <ArrowDownToLine size={13} className="text-[#c4389a]" />
              ) : (
                <ArrowUpFromLine size={13} className="text-[#c4389a]" />
              )}
              <span className="text-xs font-mono text-text">
                {truncateAddr(data.leader)}
              </span>
        ...
        {data.type === "deregister" && (
          <div className="flex items-center gap-3 bg-surface-alt rounded-md px-3 py-3">
            ...
            <div>
              ...
              <span className="text-xs text-text-muted block">
                Your staked STT will be returned
              </span>
            </div>
          </div>
        )}
        {data.type === "claimFees" && (
          <div className="flex items-center gap-3 bg-surface-alt rounded-md px-3 py-3">
            ...
            <div>
              ...
              <span className="text-xs text-text-muted block">
                From followers mirroring your trades
              </span>
            </div>
          </div>
        )}
        {data.type === "approve" && (
          <div className="space-y-2">
            <div className="flex items-center justify-between bg-surface-alt rounded-md px-3 py-2.5">
              ...
              <span className="text-xs font-bold text-text bg-primary/20 px-2 py-0.5 rounded-md">
                {data.token}
              </span>
            </div>
            <div className="flex items-center justify-between bg-surface-alt rounded-md px-3 py-2.5">
              ...
              <div className="text-right">
                <span className="text-xs font-medium text-text block">
                  {data.spender === "followerVault"
                    ? "Follower Vault"
                    : "SimpleDEX"}
                </span>
                {data.spenderAddress && (
                  <span className="text-xs font-mono text-text-muted">
                    {truncateAddr(data.spenderAddress)}
                  </span>
                )}
              </div>
            </div>
...
function TokenPill({
  ...
  return (
    <div className="flex-1 text-center">
      <span className="text-xs text-text-muted block mb-1">{sublabel}</span>
      <div
        className={`rounded-lg px-3 py-2.5 ${highlight ? "bg-primary/25 border border-primary/40" : "bg-surface-alt"}`}
      >
        <span
          className={`text-base font-bold block ${highlight ? "text-[#c4389a]" : "text-text"}`}
        >
          {amount}
        </span>
