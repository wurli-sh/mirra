...
import {
  Wallet, ArrowRight, Shield, TrendingUp, Users, BarChart3,
  Copy, Star, Trophy,
} from 'lucide-react'
    ...
    <div className="pt-16 pb-12">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-3 mb-6">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="bg-surface rounded-lg px-3 py-3 flex items-center gap-2">
...
export function TradePage() {
  ...
  const [leaderModalMode, setLeaderModalMode] = useState<'register' | 'deregister'>('register')
  ...
  const protocolCounters = [
    { icon: Users, value: statsLoading ? null : stats.leaders, label: 'Leaders' },
    { icon: TrendingUp, value: statsLoading ? null : stats.followers, label: 'Followers' },
    { icon: BarChart3, value: statsLoading ? null : stats.volume, label: 'Volume' },
  ]

  // User-specific stat (4th slot)
  const userStat = isConnected && !leaderLoading
    ? isLeader
      ? { icon: Trophy, value: leaderStats.isLoading ? null : (leaderStats.score || '—'), label: 'Your Score' }
  ...
  return (
    <div className="pt-16 pb-12">
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            {isConnected && !leaderLoading && (
              <span className={cn(
                ...
                isLeader ? 'bg-primary text-secondary' : 'bg-success/10 text-success'
              )}>
        ...
        {isConnecting || (isConnected && leaderLoading) ? (
          ...
          <motion.button
            ...
            onClick={() => { setLeaderModalMode('deregister'); setLeaderModalOpen(true) }}
            whileTap={{ scale: 0.95 }}
          >
            <Star size={12} />
            Deregister
          ...
          <motion.button
            ...
            onClick={() => { setLeaderModalMode('register'); setLeaderModalOpen(true) }}
      ...
      {/* Stats bar */}
      <motion.div
        className={cn('grid gap-2 sm:gap-3 mb-6', userStat ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-3')}
        initial={{ opacity: 0, y: 12 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.3, delay: 0.05 }}
      >
        {[...protocolCounters, ...(userStat ? [userStat] : [])].map(({ icon: Icon, value, label }) => (
          <div key={label} className="bg-surface rounded-lg px-3 sm:px-4 py-3 flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              {value === null ? (
                ...
                <span className="text-base font-bold tracking-tight block truncate">{value}</span>
              )}
            ...
            >
              <div className="border border-border rounded-xl overflow-hidden">
                {isConnecting ? (
                  ...
                  <div className="py-16 text-center">
                    <Shield size={24} className="text-text-faint mx-auto mb-3" />
                    <p className="text-sm font-medium text-text-muted">No active positions</p>
                    <p className="text-xs text-text-faint mt-1 mb-3">Follow a leader to start mirroring.</p>
                    <button
              ...
              {isConnected && positions.length > 0 && (
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {[
                    ...
                    { v: `${positions.reduce((s, p) => s + p.deposited, 0).toFixed(0)} STT`, l: 'Deposited', icon: Wallet },
                    { v: formatPnl(positions.reduce((s, p) => s + p.pnl, 0)), l: 'Total P&L', icon: TrendingUp, c: positions.reduce((s, p) => s + p.pnl, 0) >= 0 ? 'text-success' : 'text-danger' },
                  ].map(({ v, l, icon: Icon, c }) => (
                    <div key={l} className="bg-surface rounded-lg px-3 py-2.5 flex items-center gap-2">
                      <Icon size={12} className="text-text-faint shrink-0" />
                      <div className="min-w-0">
                        <span className={cn('text-sm font-bold block truncate', c)}>{v}</span>
                        <span className="text-[9px] text-text-faint uppercase tracking-wider">{l}</span>
                      </div>
                    </div>
                  ))}
                </div>
